...
def download_resume_pdf(request):
    ...
    story.append(Paragraph("Software Engineer | Building Solutions with Data Insights", 
        ...
        for cert in certifications:
            ...
            if cert.issue_date:
                ...
                if cert.expiry_date:
                    ...
                    story.append(Paragraph(f"Issued: {issue_date} | Expires: {expiry_date}", normal_style))
                else:
    ...
    if skills.exists():
        ...
        skills_by_category = {}
        for skill in skills:
            category = skill.get_category_display() if hasattr(skill, 'get_category_display') else skill.category.replace('_', ' ').title()
    ...
    response['Content-Disposition'] = 'attachment; filename="Emmanuel_Ugbaje_Resume.pdf"'
    return response
