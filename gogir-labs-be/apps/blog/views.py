...
class PostViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Post.objects.filter(published=True).select_related('author', 'category').prefetch_related('tags', 'comments')
    ...
    def related(self, request, pk=None):
        ...
        if related_posts.count() < limit and post.category:
            ...
            ).exclude(pk=post.pk).exclude(pk__in=related_posts.values_list('id', flat=True))
            related_posts = list(related_posts) + list(category_posts[:limit - related_posts.count()])
        
        # If still not enough, get recent posts
        if len(related_posts) < limit:
            recent_posts = Post.objects.filter(
                published=True
            ).exclude(pk=post.pk).exclude(
                pk__in=[p.pk for p in related_posts if hasattr(p, 'pk')]
            )[:limit - len(related_posts)]
            related_posts = list(related_posts) + list(recent_posts)
        
        serializer = PostListSerializer(related_posts[:limit], many=True, context={'request': request})
        return Response(serializer.data)
