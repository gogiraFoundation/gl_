    ...
    <p><a href="{{ content_url }}">View Content</a></p>
        ...
        for template_data in templates:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'Created template for {template_data["notification_type"]}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'Template for {template_data["notification_type"]} already exists')
                )
