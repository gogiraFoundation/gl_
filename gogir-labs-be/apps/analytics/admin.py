...
class PageViewAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['created_at', 'path', 'referer', 'user_agent', 'ip_address', 'session_id']
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['test_data_url'] = reverse('admin:analytics_pageview_create_test_data')
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('create-test-data/', self.admin_site.admin_view(self.create_test_data), name='analytics_pageview_create_test_data'),
        ]
    ...
    def create_test_data(self, request):
        ...
        test_paths = ['/', '/portfolio', '/blog', '/about', '/contact', '/portfolio/invenire']
...
class EventAdmin(admin.ModelAdmin):
    list_display = ['event_name', 'event_type', 'path_short', 'session_id', 'created_at', 'metadata_display']
    list_filter = ['event_type', 'created_at', 'event_name']
    search_fields = ['event_name', 'path', 'session_id', 'event_type']
    readonly_fields = ['created_at', 'event_type', 'event_name', 'path', 'metadata', 'user_agent', 'ip_address', 'session_id']
    date_hierarchy = 'created_at'
    ordering = ['-created_at']
    list_per_page = 50
    ...
    def metadata_display(self, obj):
        if obj.metadata:
            metadata_str = ', '.join([f"{k}: {v}" for k, v in list(obj.metadata.items())[:3]])
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['test_data_url'] = reverse('admin:analytics_event_create_test_data')
        return super().changelist_view(request, extra_context=extra_context)
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('create-test-data/', self.admin_site.admin_view(self.create_test_data), name='analytics_event_create_test_data'),
        ]
        return custom_urls + urls
    ...
    def create_test_data(self, request):
        ...
        test_events = [
            {'event_type': 'click', 'event_name': 'nav_link', 'path': '/', 'metadata': {'link': 'Home'}},
            {'event_type': 'click', 'event_name': 'nav_link', 'path': '/portfolio', 'metadata': {'link': 'Portfolio'}},
            {'event_type': 'click', 'event_name': 'blog_post_view', 'path': '/blog', 'metadata': {'postId': 1, 'postTitle': 'Test Post'}},
            {'event_type': 'form_submit', 'event_name': 'form_submit_contact', 'path': '/contact', 'metadata': {'formName': 'contact'}},
            {'event_type': 'click', 'event_name': 'social_github', 'path': '/', 'metadata': {'platform': 'github'}},
            {'event_type': 'click', 'event_name': 'project_view', 'path': '/portfolio/invenire', 'metadata': {'projectId': 1, 'projectTitle': 'Invenire'}},
        ]
